...
export const Route = createFileRoute("/portal/tools/renewal-comparison")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Compare your current renewal offer against possible mortgage options." },
    ],
  }),
  component: renewal_comparison_Tool,
});

function renewal_comparison_Tool() {
  ...
  return (
    <ToolPageShell
      ...
      onSave={() => saveScenario({ tool: "Renewal Comparison Tool", name: `${fmtMoney(a)} estimate`, inputs: { a, b }, outputs: { result } })}
