  ...
  head: () => ({
    meta: [
      { title: "Connected Accounts — approvU Settings" },
      {
        name: "description",
        content:
          "Manage banking, identity, and calendar connections that streamline your application.",
      },
    ],
  }),
...
function ConnectionsPage() {
  const [bankFlow, setBankFlow] = useState<
    { mode: "new" } | { mode: "reconnect"; id: string } | null
  >(null);
  ...
  const [productivity, setProductivity] = useState<Conn[]>([
    ...
    },
    {
      ...
      desc: "Subscribe via secure ICS feed.",
      status: "Disconnected",
      icon: CalendarDays,
    },
  ]);

  const flip = (list: Conn[], setter: (v: Conn[]) => void, id: string, target: Conn["status"]) =>
    setter(
      list.map((c) =>
        c.id === id
          ? { ...c, status: target, lastSync: target === "Connected" ? "Just now" : c.lastSync }
          : c,
      ),
    );
    ...
    <>
      <div className="space-y-6">
        <SettingPane
          ...
          desc="Connect a bank to auto-pull statements and pay history. We use Flinks; nothing is stored on our servers."
        >
          <div className="space-y-3">
            {banking.map((c) => (
              <ConnRow
                ...
                onDisconnect={() => {
                  flip(banking, setBanking, c.id, "Disconnected");
        ...
        <SettingPane
          ...
          desc="Use Google or Apple to sign in. Disconnecting won't delete your approvU account."
        >
          <div className="space-y-3">
            {identity.map((c) => (
              <ConnRow
                ...
                c={c}
                onConnect={() => {
                  flip(identity, setIdentity, c.id, "Connected");
                  toast.success(`${c.name} connected`);
                }}
                onDisconnect={() => {
                  flip(identity, setIdentity, c.id, "Disconnected");
                  toast(`${c.name} disconnected`);
                }}
              />
            ))}
        ...
        <SettingPane
          ...
          desc="Sync key dates so nothing slips between your inbox and the closing date."
        >
          <div className="space-y-3">
            {productivity.map((c) => (
              <ConnRow
                ...
                c={c}
                onConnect={() => {
                  flip(productivity, setProductivity, c.id, "Connected");
                  toast.success(`${c.name} connected`);
                }}
                onDisconnect={() => {
                  flip(productivity, setProductivity, c.id, "Disconnected");
                  toast(`${c.name} disconnected`);
                }}
              />
            ))}
      ...
      <div className="flex shrink-0 items-center gap-2">
        ...
        {c.status === "Connected" ? (
          ...
          >
            ...
            {c.status === "Action required" ? "Reconnect" : "Connect"}
          </button>
        )}
      </div>
    </div>
